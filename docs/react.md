...
## В чём отличие useEffect от useLayoutEffect?
...
## Что делает useEffect и как работает функция очистки?
...
**React** — JavaScript-библиотека для построения UI (Meta). Компонентный, декларативный, с однонаправленным потоком данных.

**Плюсы:**

- **Компонентный подход** — переиспользуемые UI-блоки, легко поддерживать и тестировать
- **Virtual DOM** — минимальные обновления реального DOM, высокая производительность
- **Огромная экосистема** — Next.js, React Native, тысячи готовых библиотек
- **Большое сообщество** — много документации, решений, специалистов на рынке
- **Гибкость** — не навязывает архитектуру, свободный выбор стейт-менеджера, роутера и т.д.
- **React Native** — переиспользование логики для мобильных приложений
- **Активное развитие** — Concurrent Mode, Server Components, постоянные улучшения

**Минусы:**

- **Только UI** — нужны дополнительные библиотеки для роутинга, стейта, запросов
- **Скорость изменений** — экосистема быстро меняется, сложно уследить за лучшими практиками
- **JSX** — смешивание разметки и логики непривычно для новичков
- **SEO-проблемы** — SPA плохо индексируется без SSR (решается Next.js)
- **Размер бандла** — при неосторожном использовании может вырасти

```jsx
// React — только View, всё остальное — ваш выбор
import { BrowserRouter } from "react-router-dom"; // роутинг отдельно
import { create } from "zustand"; // стейт отдельно
import { useQuery } from "@tanstack/react-query"; // запросы отдельно
```

## Как работают useMemo и useCallback для оптимизации React?
...
## Как работает алгоритм Reconciliation (согласования)?
...
Движок reconciliation — **React Fiber** (React 16+), подробнее в отдельном вопросе ниже.

## Как работает useRef и для чего используется?
...
## Как избежать лишних перерендеров в React?
...
## Что такое Redux и как он устроен?
...
const counterSlice = createSlice({
  ...
  reducers: {
    increment: (state) => {
      state.value += 1;
    }, // RTK использует Immer — мутации OK
    decrement: (state) => {
      state.value -= 1;
    },
  },
...
## В чём разница Redux, Zustand и MobX?
...
## В чём разница функциональных и классовых компонентов?
...
## Что такое ключ (key) в React и зачем он нужен?
...
## Что такое Stale Closure в useEffect?
...
## Как работают React.lazy и Suspense?
...
## Что такое useTransition и useDeferredValue?
