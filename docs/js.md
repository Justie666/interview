...
## Что такое Event Loop и как он работает с микрозадачами и макрозадачами?

**Event Loop** — механизм, позволяющий JavaScript (однопоточному языку) выполнять асинхронный код без блокировки потока.

**Как задачи путешествуют по системе:**

1. **Синхронный код** — попадает прямо в Call Stack и выполняется сразу, строчка за строчкой
2. **Асинхронные вызовы** (`setTimeout`, `fetch`, DOM-события) — передаются в **Web APIs** (среда браузера/Node.js), которые работают вне JS-потока. Там идёт таймер, сетевой запрос и т.д.
3. Когда Web API завершает работу — коллбэк кладётся в очередь:
   - `Promise.then`, `queueMicrotask` → **Microtask Queue**
   - `setTimeout`, `setInterval`, UI-события → **Macrotask Queue**
4. **Event Loop** следит за Call Stack: как только он пуст — сначала вычищает **всю** Microtask Queue, затем берёт **одну** задачу из Macrotask Queue, снова вычищает микрозадачи — и так по кругу
...
## Что такое область видимости (scope) и замыкание в JavaScript?
...
**TDZ (Temporal Dead Zone)** — зона между началом блока и строкой объявления `let`/`const`, где переменная уже существует в scope, но обратиться к ней нельзя. JS знает о ней (она «поднята»), но намеренно запрещает доступ до инициализации.

```js
{
  // TDZ для y начинается здесь ↓
  console.log(y); // ReferenceError — переменная в TDZ
  let y = 5; // TDZ заканчивается здесь, y = 5
  console.log(y); // 5 — теперь доступна
}
```

В отличие от `var` (который даёт `undefined` до присвоения), TDZ делает ошибку явной — это помогает поймать баги с использованием переменной раньше её объявления.

**Scope chain** — при обращении к переменной JS ищет сначала в текущем scope, затем во внешних, вплоть до глобального.

## В чём разница между var, const и let?
...
## В чём разница == и ===? Что такое приведение типов?
...
"5" -
  3 + // 2     (строка → число, - арифметический)
  true + // 1
  null + // 0
  undefined; // NaN
```
...
## В чём разница function declaration, function expression и стрелочной функции?
...
## В чём разница стрелочной и обычной функции в контексте this?
...
class HttpClient {
  get(url) {
    /* retry, timeout, headers, parsing — скрыто */
  }
}
...
| Буква | Принцип                                              | Суть                                                  |
| ----- | ---------------------------------------------------- | ----------------------------------------------------- |
| **S** | Single Responsibility / Единственной ответственности | Один класс/модуль — одна зона ответственности         |
| **O** | Open/Closed / Открытости-закрытости                  | Открыт для расширения, закрыт для изменения           |
| **L** | Liskov Substitution / Подстановки Лисков             | Потомок взаимозаменяем с родителем                    |
| **I** | Interface Segregation / Разделения интерфейсов       | Много узких интерфейсов лучше одного широкого         |
| **D** | Dependency Inversion / Инверсии зависимостей         | Зависеть от абстракций, а не от конкретных реализаций |
...
class OrderService {
  ...
  placeOrder() {
    this.notifier.send("Заказ оформлен");
  }
}
...
## В чём разница поверхностного и глубокого копирования объектов?
...
## В чём разница throttle и debounce?
...
## В чём разница ES Modules и CommonJS?
...
## Какие паттерны проектирования чаще всего встречаются в JavaScript?
...
## Что такое генераторы и Symbol.iterator?
